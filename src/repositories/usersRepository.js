...
function createUser({ name, email, token_id }) {
  ...
  const info = stmt.run({ name: name || null, email, token_id: token_id || null, created_at: now(), updated_at: now() });
  return info.lastInsertRowid;
}

function updateUserPartial(id, fields) {
  const allowed = ['name','email','phone_number','is_verified','date_of_birth','age','gender','favorite_spot','interest','distance_preference','photos','token_id'];
  ...
  const stmt = db.prepare(`UPDATE users SET ${sets}, updated_at = @updated_at WHERE id = @id`);
